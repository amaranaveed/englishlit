...
import {
  GEO_ASSESSMENT_OBJECTIVES,
  GEO_PAPER_AO_BREAKDOWN,
  type GeoAODetail,
} from "@/data/geography/assessment-objectives";
...
export default function GeographyAOPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <PageBanner
        ...
        subtitle="AQA GCSE Geography 8035"
      ...
      <motion.div
        ...
        viewport={{ once: true }}
      >
        {GEO_ASSESSMENT_OBJECTIVES.map((ao) => {
      ...
      <AnimatePresence mode="wait">
        {expandedAO && (
          <GeoAODetailCard
            key={expandedAO}
            ao={GEO_ASSESSMENT_OBJECTIVES.find((a) => a.id === expandedAO)!}
          />
        )}
      </AnimatePresence>
      ...
      >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm font-ui">
              ...
              <motion.tbody
                ...
                viewport={{ once: true }}
              >
                {GEO_PAPER_AO_BREAKDOWN.map((row, i) => (
                  <motion.tr
                    key={i}
                    variants={staggerItem}
                  ...
                  >
                    ...
                    <td className="text-center px-3 py-3 text-blue font-semibold">{row.ao3}</td>
                    <td className="text-center px-3 py-3 text-green font-semibold">{row.ao4}</td>
        ...
        <p className="font-ui text-xs text-grey mt-2">
          Paper 3 (Geographical Applications) is the pre-release and fieldwork paper. AO3 is heavily weighted here because of the decision-making and evaluation questions.
        </p>
      </motion.section>
      ...
      <div className="flex items-center justify-between">
        ...
        <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.95 }}>
          <Link href="/geography/essay-structure" className="font-ui text-sm text-grey hover:text-text transition-colors">
            Essay Structure &rarr;
          </Link>
        </motion.div>
      </div>
    </div>
...
function GeoAODetailCard({ ao }: { ao: GeoAODetail }) {
  const colourMap: Record<string, { border: string; heading: string; badge: string }> = {
    ...
    blue:   { border: "border-blue/20",   heading: "text-blue",   badge: "bg-blue text-white" },
    green:  { border: "border-green/20",  heading: "text-green",  badge: "bg-green text-white" },
  };
          ...
          {ao.sentenceStarters.map((ss, j) => (
            <span
              ...
              className={`font-ui text-xs italic px-3 py-1.5 rounded-lg ${
                ao.colour === "teal" ? "bg-teal-light text-teal" :
                ao.colour === "purple" ? "bg-purple-light text-purple" :
                ao.colour === "blue" ? "bg-blue-light text-blue" :
