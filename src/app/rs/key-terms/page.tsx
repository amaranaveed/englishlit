...
const CATEGORY_COLOURS: Record<string, { bg: string; text: string }> = {
  "key-term": { bg: "bg-blue-100", text: "text-blue-700" },
  teaching: { bg: "bg-indigo-100", text: "text-indigo-700" },
  scripture: { bg: "bg-violet-100", text: "text-violet-700" },
  "divergent-view": { bg: "bg-sky-100", text: "text-sky-700" },
};
...
export default function RsKeyTermsPage() {
  ...
  const topics = useMemo(() => getActiveRsTopics(), []);
  const allTerms = useMemo(() => getUniqueRsTerms(), []);

  useEffect(() => {
    getVocabScores().then((all) => {
      setScores(all.filter((s) => s.textSlug.startsWith("rs-") || s.textSlug === "all-rs"));
    });
  }, [getVocabScores]);

  const quizTerms = useMemo(() => {
    const rsTerms = selectedTopic === "all" ? getUniqueRsTerms() : getRsTermsByTopic(selectedTopic);
    return rsTermsToVocabTerms(rsTerms);
  }, [selectedTopic]);
  ...
  async function handleComplete(correct: number, total: number, missed: string[]) {
    const score: VocabQuizScore = {
      ...
      textSlug: selectedTopic === "all" ? "all-rs" : `rs-${selectedTopic}`,
      mode,
      total,
      correct,
      missed,
    ...
    setScores(updatedScores.filter((s) => s.textSlug.startsWith("rs-") || s.textSlug === "all-rs"));
    setStage("results");
  }
      ...
      <div className="flex rounded-lg bg-grey-light p-1 mb-6">
        <motion.button
          ...
          className={`flex-1 rounded-md py-2 font-ui text-sm font-semibold transition-colors cursor-pointer ${
            topTab === "browse" ? "bg-surface text-blue-600 shadow-sm" : "text-grey hover:text-text"
          }`}
        >
          Browse
        </motion.button>
        <motion.button
          ...
          className={`flex-1 rounded-md py-2 font-ui text-sm font-semibold transition-colors cursor-pointer ${
            topTab === "quiz" ? "bg-surface text-blue-600 shadow-sm" : "text-grey hover:text-text"
          }`}
        >
          Quiz
        </motion.button>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-surface p-5">
            ...
            >
              ...
              {topics.map((topic) => {
                const count = getRsTermsByTopic(topic.slug).length;
                return (
                  <motion.button
                    key={topic.slug}
                    variants={staggerItem}
            ...
            >
              ...
              <div className="space-y-2">
                {scores.slice(0, 5).map((s) => {
                  ...
                  const topicLabel = s.textSlug === "all-rs" ? "All topics" : s.textSlug.replace("rs-", "");
...
function BrowseTab({
  ...
  const topics = useMemo(() => getActiveRsTopics(), []);
  const allTerms = useMemo(() => getAllRsTerms(), []);
  ...
  const grouped = useMemo(() => {
    return topics.map((topic) => {
      ...
      const categories = ["key-term", "teaching", "scripture", "divergent-view"] as const;
    ...
    <div>
      ...
      >
        <input
          ...
          className="w-full rounded-lg border border-border bg-surface px-4 py-2.5 font-ui text-sm text-text placeholder:text-grey focus:outline-none focus:ring-2 focus:ring-blue-500/30 mb-2"
        />
