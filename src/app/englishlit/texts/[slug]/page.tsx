    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {TEXT_IMAGES[slug] ? (
        <AnimatedBreadcrumb>
          <nav className="font-ui text-[13px] text-grey mb-3">
            <Link href="/englishlit/texts" className="hover:text-foreground transition-colors">
              Texts
            </Link>
        ...
        <div className="relative rounded-2xl overflow-hidden bg-[#1a1b3a]"
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-8 sm:px-10">
            <nav className="font-ui text-[13px] text-white/50 mb-3">
              <Link href="/englishlit/texts" className="hover:text-white/80 transition-colors">
                Texts
              </Link>
              <span className="mx-2">›</span>
      ...
      <StaggerGrid className="grid sm:grid-cols-2 gap-4 mb-10">
        <AnimatedInteractiveCard index={0}>
          <AnimatedLinkCard>
            <Link
              href={`/englishlit/texts/${slug}/quotes`}
        ...
        {hasCharacters && (
          <AnimatedInteractiveCard index={1}>
            <AnimatedLinkCard>
              <Link
                href={`/englishlit/texts/${slug}/characters/${encodeURIComponent(characters[0].name)}`}
                className="card-hover rounded-xl border border-border bg-surface p-6 block"
              >
        ...
        {hasMindMapData && (
          <AnimatedInteractiveCard index={2}>
            <AnimatedLinkCard>
              <Link
                href={`/englishlit/texts/${slug}/mindmaps`}
                className="card-hover rounded-xl border border-border bg-surface p-6 block"
              >
        ...
        {hasThemeSheetData && (
          <AnimatedInteractiveCard index={3}>
            <AnimatedLinkCard>
              <Link
                href={`/englishlit/texts/${slug}/themes`}
                className="card-hover rounded-xl border border-border bg-surface p-6 block"
              >
        ...
        {hasToolkit && toolkit && (
          <AnimatedInteractiveCard index={4}>
            <AnimatedLinkCard>
              <Link
                href={`/englishlit/texts/${slug}/writers-toolkit`}
                className="card-hover rounded-xl border border-border bg-surface p-6 block"
              >
        ...
        {slug === "inspector-calls" && (
          <AnimatedInteractiveCard index={6}>
            <AnimatedLinkCard>
              <Link
                href={`/englishlit/texts/${slug}/exam-prep`}
              ...
              <div className="flex flex-wrap gap-2">
                {overview.themes.map((theme, i) => (
                  <AnimatedBadge key={theme} delay={i * 0.05}>
                    <Link
                      href={`/englishlit/texts/${slug}/quotes?theme=${encodeURIComponent(theme)}`}
            ...
            <StaggerGrid className="grid sm:grid-cols-2 gap-3">
              {overview.characters.map((c, i) => {
                ...
                const href = hasAnalysis
                  ? `/englishlit/texts/${slug}/characters/${encodeURIComponent(c.name)}`
                  : `/englishlit/texts/${slug}/quotes?who=${encodeURIComponent(c.name)}`;
                return (
