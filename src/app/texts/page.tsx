            ...
            <section>
              ...
              <StaggerGrid className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {group.texts.map((t) => {
                  ...
                  return active ? (
                    <AnimatedLinkCard key={t.slug}>
                      <Link href={`/texts/${t.slug}`} className="rounded-xl border border-border bg-surface p-5 card-hover block">
                        {inner}
                      </Link>
                    </AnimatedLinkCard>
                  ) : (
                    <AnimatedCard key={t.slug}>
                      <div className="rounded-xl border border-border bg-surface p-5 coming-soon-overlay opacity-60">
                        {inner}
                      </div>
                    </AnimatedCard>
                  );
              ...
              {/* Reveal line divider between sections */}
              {gi < TEXT_REGISTRY.length - 1 && (
                <AnimatedRevealLine className="h-px bg-border/50 mt-8" delay={0.2} />
              )}
            </section>
          </AnimatedSection>
        ))}
